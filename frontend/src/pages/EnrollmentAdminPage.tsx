...
export function EnrollmentAdminPage() {
  ...
  const inputCls =
    'h-11 w-full rounded-lg border border-[#26385d] bg-[#0a1120] px-3 text-sm text-[#f7f9ff] outline-none transition placeholder:text-[#7887a3] focus:border-[#8eb0ff]'

  return (
    <div className="min-h-screen bg-[#060b18] px-4 py-8 text-[#eaf0ff]">
      <div className="mx-auto w-full max-w-3xl">
        <div className="mb-6 flex items-center gap-2">
          <ShieldCheck className="size-5 text-[#8eb0ff]" />
          <h1 className="text-xl font-semibold">Secure enrollment links</h1>
        </div>

        <form onSubmit={(e) => void generate(e)} className="space-y-3 rounded-2xl border border-white/10 bg-white/[0.04] p-5">
          <p className="text-xs text-[#9db0d6]">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              <label className="mb-1 block text-xs text-[#cdd9f5]">Prospect name (watermark)</label>
              <input className={inputCls} value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Rahul Sharma" />
            </div>
            <div>
              <label className="mb-1 block text-xs text-[#cdd9f5]">Prospect phone (watermark)</label>
              <input className={inputCls} value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="10-digit number" inputMode="numeric" />
            </div>
          </div>
          <div>
            <label className="mb-1 block text-xs text-[#cdd9f5]">Video source (R2 object key or URL)</label>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              <label className="mb-1 block text-xs text-[#cdd9f5]">Title</label>
              <input className={inputCls} value={title} onChange={(e) => setTitle(e.target.value)} />
            </div>
            <div>
              <label className="mb-1 block text-xs text-[#cdd9f5]">Window (minutes)</label>
              <input
                className={inputCls}
                type="number"
                min={1}
          ...
          {error ? <p className="text-sm text-[#ffb8bd]">{error}</p> : null}
          <button
            type="submit"
            disabled={busy || !videoSource.trim()}
            className="inline-flex h-11 items-center justify-center gap-2 rounded-lg bg-[#dce7ff] px-5 text-sm font-semibold text-[#0a1530] transition hover:bg-[#c6d8ff] disabled:opacity-60"
          >
        ...
        <div className="mt-6 space-y-2">
          <h2 className="text-sm font-semibold text-[#9db0d6]">Recent links</h2>
          {rows.length === 0 ? (
            <p className="text-xs text-[#6c7da0]">No links yet.</p>
          ) : (
            rows.map((r) => (
              <div key={r.token} className="flex items-center justify-between gap-3 rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2.5">
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium">{r.viewer_name || 'No name'} · {r.title || 'Enrollment video'}</p>
                  <p className="truncate text-[11px] text-[#7e90b6]">
                ...
                <div className="flex shrink-0 items-center gap-1.5">
                  <button
                    ...
                    className="inline-flex size-8 items-center justify-center rounded-md bg-[#1a2942] text-[#cdd9f5] hover:bg-[#22344f]"
                    title="Copy link"
                  >
                    {copied === r.token ? <Check className="size-4 text-emerald-400" /> : <Copy className="size-4" />}
                  </button>
                  <button
                    type="button"
                    onClick={() => void revoke(r.token)}
                    className="inline-flex size-8 items-center justify-center rounded-md bg-[#3a1a1f] text-[#ffb8bd] hover:bg-[#4a2228]"
                    title="Revoke"
                  >
