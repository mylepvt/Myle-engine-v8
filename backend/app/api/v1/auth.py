...
async def register(
    ...
    r_existing = await session.execute(
        select(User.id).where(User.fbo_id == fbo_stored)
    )
...
async def dev_login(
    ...
    if user is None:
        by_fbo = await session.execute(
            select(User).where(User.fbo_id == fbo_id)
        )
    ...
    if user is None:
        ...
        await session.commit()
        await session.refresh(user)
    await ensure_user_compliance_snapshot(
        session, user_id=user.id, apply_actions=True
    )
    await session.refresh(user)
...
async def change_password(
    ...
    if not db_user.hashed_password or not verify_password_legacy_compatible(
    ...
    await session.commit()
    return ChangeCredentialResponse(
