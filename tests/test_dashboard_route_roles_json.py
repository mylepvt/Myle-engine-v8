"""Keep `config/dashboard-route-roles.json` aligned with product roles (no drift / typos)."""
...
_JSON = _ROOT / "config" / "dashboard-route-roles.json"
...
def test_dashboard_route_roles_json_exists_and_valid() -> None:
    ...
    assert data, "dashboard-route-roles.json is empty"
