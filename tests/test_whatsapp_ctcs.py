...
def test_interested_no_webhook_is_stub(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    out = asyncio.run(send_interested_flp_min_billing_assets(lead_id=42, phone="+919876543210"))
...
def test_interested_webhook_ok(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    out = asyncio.run(send_interested_flp_min_billing_assets(lead_id=7, phone="999"))
    assert out["ok"] is True
