...
Gate applies to non-admin/non-leader roles (team members).
"""
...
@pytest.mark.asyncio
async def test_day3_blocked_until_batches_and_test(
    admin_client: AsyncClient, team_client: AsyncClient, engine
):
    # Create lead using admin
    resp = await admin_client.post(
        "/api/v1/leads",
        json={"name": "D2", "status": "new_lead", "phone": str(next(_PHONE))},
    )
    lead_id = resp.json()["id"]

    # Advance to Day 2 directly via DB and assign to team user
    async with AsyncSession(engine, expire_on_commit=False) as s:
        await s.execute(
            update(Lead)
            .where(Lead.id == lead_id)
            .values(status="day2", assigned_to_user_id=201)
        )
        await s.commit()

    # nothing done -> blocked (batches first)
    r1 = await team_client.patch(
        f"/api/v1/leads/{lead_id}", json={"status": "day3"}
    )
