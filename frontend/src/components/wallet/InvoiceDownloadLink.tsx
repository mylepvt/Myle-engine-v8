...
export function InvoiceDownloadLink({ invoiceNumber, kind, amountCents, className }: Props) {
  ...
  return (
    <a
      ...
      rel="noreferrer"
      className={cn(
        'inline-flex max-w-full shrink-0 items-center gap-1.5 rounded-lg border border-white/12 bg-muted/60 px-2.5 py-1.5 text-left text-ds-caption font-medium text-primary shadow-glass-inset transition-colors hover:border-primary/40 hover:bg-primary/10',
        className,
      )}
